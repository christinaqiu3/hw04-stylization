...
/*
This is a custom implementation based on Kristin Lague's project. 
I referenced this repo: https://github.com/KristinLague/Mesh-Cutting
and this video: https://www.youtube.com/watch?v=1UsuZsaUUng&t=7s 
My changes are marked with "CHANGE" comments.
*/
...
public class Cutter : MonoBehaviour
{
    ...
    public static GameObject Cut(GameObject originalGameObject, Vector3 contactPoint, Vector3 cutNormal)
    {
        ...
        // CHANGE: Keep plane in WORLD SPACE
        Plane cutPlane = new Plane(cutNormal, contactPoint);
        ...
        // CHANGE: Transform vertices to world space inside SeparateMeshes
        SeparateMeshes(leftMesh, rightMesh, cutPlane, addedVertices, originalGameObject.transform);
        //SeparateMeshes(leftMesh,rightMesh,cutPlane,addedVertices);

        FillCut(addedVertices, cutPlane, leftMesh, rightMesh);

        // CHANGE: Convert back to LOCAL space before creating mesh
        Transform backTransform = originalGameObject.transform;
        for (int i = 0; i < leftMesh.Vertices.Count; i++)
            leftMesh.Vertices[i] = backTransform.InverseTransformPoint(leftMesh.Vertices[i]);

        for (int i = 0; i < rightMesh.Vertices.Count; i++)
            rightMesh.Vertices[i] = backTransform.InverseTransformPoint(rightMesh.Vertices[i]);
        // change end
    ...
    // CHANGE: Made objectTransform parameter to convert vertices to world space
    /// <summary>
    ...
    private static void SeparateMeshes(GeneratedMesh leftMesh, GeneratedMesh rightMesh, Plane plane, List<Vector3> addedVertices, Transform objectTransform)
    {
        // CHANGE: Use objectTransform to convert vertices to world space
        ...
        for (int i = 0; i < originalMesh.subMeshCount; i++)
        {
            ...
            for (int j = 0; j < subMeshIndices.Length; j += 3)
            {
                ...
                // CHANGE: Convert to WORLD space before plane test
                Vector3 vA_world = objectTransform.TransformPoint(vertices[triangleIndexA]);
                Vector3 vB_world = objectTransform.TransformPoint(vertices[triangleIndexB]);
                Vector3 vC_world = objectTransform.TransformPoint(vertices[triangleIndexC]);

                // CHANGE: Create MeshTriangle in world space
                MeshTriangle currentTriangle = new MeshTriangle(
                    new[] { vA_world, vB_world, vC_world },
                    new[] { objectTransform.TransformDirection(normals[triangleIndexA]),
                            objectTransform.TransformDirection(normals[triangleIndexB]),
                            objectTransform.TransformDirection(normals[triangleIndexC]) },
                    new[] { uvs[triangleIndexA], uvs[triangleIndexB], uvs[triangleIndexC] },
                    i
                );
                // GetTriangle(triangleIndexA,triangleIndexB,triangleIndexC,i);

                // CHANGE: use world space vertices for plane test, original code used local space
                ...
                bool triangleALeftSide = plane.GetSide(vA_world);// originalMesh.vertices[triangleIndexA]);
                bool triangleBLeftSide = plane.GetSide(vB_world);
                bool triangleCLeftSide = plane.GetSide(vC_world);

                // CHANGE: Classify and add to the correct mesh
